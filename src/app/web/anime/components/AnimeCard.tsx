...
import { AnimePreviewModal } from "./AnimePreviewModal";

/**
 * Tarjeta poster de un título real (ManglarAnime / ManglarHentai, vía
 * anime1v-api). Mismo patrón que MovieCard: click abre <AnimePreviewModal>
 * en vez de mandar directo afuera.
 */
export function AnimeCard({ item, domain }: { item: EcosystemAnimeItem; domain: string }) {
  ...
  return (
    <>
      ...
      {open && <AnimePreviewModal item={item} domain={domain} onClose={() => setOpen(false)} />}
    </>
  );
}
