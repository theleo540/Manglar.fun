...
/**
 * Vista previa antes de mandar afuera — mismo comportamiento que
 * MoviePreviewModal. "Ver ahora" manda a `${domain}/?title=${item.id}`;
 * el frontend real (App.tsx -> parseLocationState) ya lee ese query param
 * al montar y abre el detalle directo, porque item.id ya viene armado
 * por el backend con el mismo esquema "anime-<base64url(url)>" que usa
 * lib/animeId.ts ahí (ver src/utils/animeId.js en anime1v-api). No hay
 * ningún id/mapeo inventado acá — es el mismo id que el sitio real sabe
 * decodificar.
 */
export function AnimePreviewModal({
  item,
  domain,
  onClose,
}: {
  item: EcosystemAnimeItem;
  domain: string;
  onClose: () => void;
}) {
        ...
        <div className="px-5 md:px-7 pt-5 pb-6">
          ...
          {item.type && (
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <span className="text-xs bg-white/10 text-white/80 px-2 py-0.5 rounded-sm">{item.type}</span>
            </div>
          )}

          <p className="text-white/40 text-xs leading-relaxed">
            Al hacer click en "Ver ahora" te llevamos directo a este título.
          </p>
        </div>
      </div>
    </div>,
